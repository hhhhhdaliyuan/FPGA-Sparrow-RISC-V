...
static inline void freeze(void)   { *(volatile uint32_t*)0x40000400 = 1; }
static inline void unfreeze(void) { *(volatile uint32_t*)0x40000400 = 0; }
...
void find_largest_blob(void) {
    ...
    for (int y = SCAN_Y0; y <= SCAN_Y1; y++) {
        for (int x = 0; x < IMG_W; x++) {
            ...
            // 欧氏距离长宽比
            int dw = trx-tlx, dh_w = try_-tly;
            int dh = blx-tlx, dh_h = bly-tly;  // 用左边
            int w2 = dw*dw + dh_w*dh_w;
            int h2 = dh*dh + dh_h*dh_h;

            if (area > best_area && area >= MIN_AREA &&
                w2 >= MIN_W*MIN_W && h2 >= MIN_H*MIN_H &&
                w2 >= h2*(MIN_RATIO*MIN_RATIO) &&
                w2 <= h2*(MAX_RATIO*MAX_RATIO)) {
    ...
    if(best_area>=MIN_AREA){
        ...
        printf("四角(RAW估计):\r\n");
        printf("  左上(%d,%d) 右上(%d,%d)\r\n",best_tlx+PIPE_DX,best_tly+PIPE_DY,best_trx+PIPE_DX,best_try+PIPE_DY);
        printf("  左下(%d,%d) 右下(%d,%d)\r\n",best_blx+PIPE_DX,best_bly+PIPE_DY,best_brx+PIPE_DX,best_bry+PIPE_DY);
    }else{printf("未找到\r\n");}
    printf("===== END =====\r\n\n");
}
...
void send_raw_blob(void) {
    ...
    int rx0=g_blob_x0+PIPE_DX, ry0=g_blob_y0+PIPE_DY;
    int rx1=g_blob_x1+PIPE_DX, ry1=g_blob_y1+PIPE_DY;
