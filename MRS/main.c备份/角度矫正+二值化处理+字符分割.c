...
void morph_close_bitmap(int w, int h) {
    ...
    int wpl = w >> 2;
    int bm_words = (w + 31) >> 5;
    volatile uint32_t* bm = (volatile uint32_t*)0xA1B00000;
    volatile uint32_t* tmp = (volatile uint32_t*)0xA1B01000;
    // 8bit -> 1bit位图
    for (int y = 0; y < h; y++) {
        for (int xb = 0; xb < w; xb += 32) {
            uint32_t bits = 0;
            int wk = xb >> 5;
            for (int k = 0; k < 32 && (xb + k) < w; k++) {
                int bi = y * wpl + ((xb + k) >> 2);
                int bp = ((xb + k) & 3) * 8;
                if ((bw[bi] >> bp) & 0xFF) bits |= (1 << k);
            }
            bm[y * bm_words + wk] = bits;
        }
    }
    // 水平膨胀
    for (int y = 0; y < h; y++) {
        ...
        for (int wd = 0; wd < bm_words; wd++) {
            uint32_t c = bm[r + wd]; uint32_t d = c;
            if (wd > 0) { d |= (c >> 1) | (bm[r + wd - 1] << 31); } else { d |= c >> 1; }
            if (wd < bm_words - 1) { d |= (c << 1) | (bm[r + wd + 1] >> 31); } else { d |= c << 1; }
            tmp[r + wd] = d;
        }
    }
    // 垂直膨胀
    for (int y = 0; y < h; y++) {
        int r = y * bm_words; int ru = (y > 0) ? (y - 1) * bm_words : r; int rd = (y < h - 1) ? (y + 1) * bm_words : r;
        for (int wd = 0; wd < bm_words; wd++) { tmp[r + wd] |= tmp[ru + wd] | tmp[rd + wd]; }
    }
    // 水平腐蚀
    for (int y = 0; y < h; y++) {
        int r = y * bm_words;
        for (int wd = 0; wd < bm_words; wd++) {
            uint32_t c = tmp[r + wd]; uint32_t e = c;
            if (wd > 0) { e &= (c >> 1) | (tmp[r + wd - 1] << 31); } else { e &= c >> 1; }
            if (wd < bm_words - 1) { e &= (c << 1) | (tmp[r + wd + 1] >> 31); } else { e &= c << 1; }
            bm[r + wd] = e;
        }
    }
    // 垂直腐蚀
    for (int y = 0; y < h; y++) {
        int r = y * bm_words; int ru = (y > 0) ? (y - 1) * bm_words : r; int rd = (y < h - 1) ? (y + 1) * bm_words : r;
        for (int wd = 0; wd < bm_words; wd++) { bm[r + wd] &= bm[ru + wd] & bm[rd + wd]; }
    }
    // 位图 -> 8bit回写
    for (int y = 0; y < h; y++) {
        for (int xb = 0; xb < w; xb += 32) {
            uint32_t bits = bm[y * bm_words + (xb >> 5)];
            for (int k = 0; k < 32 && (xb + k) < w; k++) {
                int bi = y * wpl + ((xb + k) >> 2);
                int bp = ((xb + k) & 3) * 8;
                uint8_t v = (bits & (1 << k)) ? 255 : 0;
                bw[bi] = (bw[bi] & ~(0xFF << bp)) | (v << bp);
            }
        }
    }
}
